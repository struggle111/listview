 ...
 * 
 * <p>
 * To use, pass in a {@link android.widget.ListAdapter} that generates
 * {@link android.widget.ImageView}s in the layout hierarchy of getView().
 * ImageViews are searched for using the IDs specified in {@code imageViewIDs}.
 * When found, {@link android.widget.ImageView#getTag(R.id.ic__uri)} is called
 * and should return a {@link android.net.Uri} referencing a local or remote
 * image. See {@link ImageCache#loadImage(int, android.net.Uri, int, int)} for
...
public class ImageLoaderAdapter extends AdapterWrapper implements
	 ...
	 * 
	 * @param context
	 *            a context for getting the display density. You don't need to
	 *            worry about this class holding on to a reference to this: it's
	 *            only used in the constructor.
	 * @param wrapped
	 *            the adapter that's wrapped. See {@link ImageLoaderAdapter} for
	 *            the requirements of using this adapter wrapper.
	 * @param cache
	 *            an instance of your image cache. This can be shared with the
	 *            process.
	 * @param imageViewIDs
	 *            a list of resource IDs matching the ImageViews that should be
	 *            scanned and loaded.
	 * @param defaultWidth
	 *            the default maximum width, in the specified unit. This size
	 *            will be used if the size cannot be obtained from the view.
	 * @param defaultHeight
	 *            the default maximum height, in the specified unit. This size
	 *            will be used if the size cannot be obtained from the view.
	 * @param unit
	 *            one of {@link #UNIT_PX} or {@link #UNIT_DIP}
	 */
	public ImageLoaderAdapter(Context context, ListAdapter wrapped,
			ImageCache cache, int[] imageViewIDs, int defaultWidth,
			int defaultHeight, int unit) {
		this(context, wrapped, cache, imageViewIDs, defaultWidth,
				defaultHeight, unit, true);
	}
	List<?> list_data=null;
	public ImageLoaderAdapter(Context context, ListAdapter wrapped,
			ImageCache cache, int[] imageViewIDs, int defaultWidth,
			int defaultHeight, int unit,List<?> ls) {
		this(context, wrapped, cache, imageViewIDs, defaultWidth,
				defaultHeight, unit, true);
		this.list_data=ls;
	}
	 ...
	 *            one of {@link #UNIT_PX} or {@link #UNIT_DIP}
	 * @param autosize
	 *            if true, the view's dimensions will be cached the first time
	 *            it's loaded and an image of the appropriate size will be
	 *            requested the next time an image is loaded. False uses
	 *            defaultWidth and defaultHeight only.
	 */
	public ImageLoaderAdapter(Context context, ListAdapter wrapped,
			ImageCache cache, int[] imageViewIDs, int defaultWidth,
			int defaultHeight, int unit, boolean autosize) {
		super(wrapped);
	...
	/**
	 * This can be called from your {@link android.app.Activity#onResume()}
	 * method.
	 */
	...
	/**
	 * This can be called from your {@link android.app.Activity#onPause()}
	 * method.
	 */
...
//		------begin--
		...
		/**if ( this.list_data!=null) {
			if (this.list_data.get(0) instanceof UserSearchVo) {
				TextView user_name = (TextView) convertView
						.findViewById(R.id.cell_username);
				TextView user_age = (TextView) convertView
						.findViewById(R.id.cell_userage);
				TextView user_height = (TextView) convertView
						.findViewById(R.id.cell_userheight);
				TextView user_pos = (TextView) v
						.findViewById(R.id.cell_user_cityand_province);
				TextView user_heart_desc = (TextView) convertView
						.findViewById(R.id.cell_desc);
				ImageView user_header=(ImageView)convertView.findViewById(R.id.thumb);
				int i=position;
					UserSearchVo usv=(UserSearchVo)this.list_data.get(i);
					String pos_Name = usv.getProvince() + usv.getCity();
					
					if (TextUtils.isEmpty(pos_Name) || "null".equals(pos_Name)) {
						GeneralUtil.setValueToView(user_pos, "四川成都");
					} else {
						
						GeneralUtil.setValueToView(user_pos, pos_Name);
					}
					if (TextUtils.isEmpty(usv.getHeart_desc())
							|| "null".equals(usv.getHeart_desc())) {
						GeneralUtil.setValueToView(user_heart_desc,
								"这是一个神秘的人物，还没有描述哦！");
					} else {
					...
					if (TextUtils.isEmpty(usv.getNickName())
						...
						GeneralUtil.setValueToView(user_name, "小西");
					} else {
						if (usv.getNickName().length() > 6) {
							user_name.setTextSize(15);
						}
						
						GeneralUtil.setValueToView(user_name,
								usv.getNickName());
					}
					GeneralUtil.setValueToView(user_age, usv.getAge() + "岁");
					if (usv.getSex().trim().equals("2")) {
						user_header
						.setImageResource(R.drawable.female_default);
						
					} else {
						user_header.setImageResource(R.drawable.man_default);
					}
...
//		------end---
		...
		for (final int id : mImageViewIDs) {
			...
			// ic__load_id is used to keep track of what load ID is associated
			// with what
			...
			Drawable d = null;
			try {
				if (viewDimension != null && viewDimension.width > 0
						&& viewDimension.height > 0) {
					d = mCache.loadImage(imageID, tag, viewDimension.width,
							viewDimension.height);
				} else {
					d = mCache.loadImage(imageID, tag, mDefaultWidth,
							mDefaultHeight);
				}
			...
			if (d != null) {
				...
				if (ImageCache.DEBUG) {
					Log.d(TAG, "scheduling load with ID: " + imageID + "; URI;"
							+ tag);
				}
				mImageViewsToLoad
						.put(imageID, new SoftReference<ImageView>(iv));
			}
