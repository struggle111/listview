...
public class SimpleThumbnailCursorAdapter extends SimpleCursorAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (mShowIndeterminate) {
			...
			if (mIndeterminate == null) {
				mIndeterminate = LayoutInflater.from(mContext).inflate(
	...
	public void changeCursor(Cursor cursor) {
		...
		setIndeterminateLoading(cursor == null
				|| isNotShowingExpectedCount(cursor));
	}

	private boolean isNotShowingExpectedCount(Cursor c) {
		return c != null
				&& (mExpectedCount > -1 && (c.getCount() != mExpectedCount));
	}

	public void setExpectedCount(int expectedCount) {
		mExpectedCount = expectedCount;
		setIndeterminateLoading(mCursor == null
				|| isNotShowingExpectedCount(mCursor));
	}
